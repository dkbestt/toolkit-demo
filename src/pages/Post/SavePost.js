...
import { getMySavePost, removeMySavePostData } from '../../store/reducers/postSlice'
...
const SavePost = () => {
    const [mySavePost, setMySavePost] = useState([])
    ...
    useEffect(() => {
        ...
        dispatch(getMySavePost(postData)).unwrap().then((res) => {
            if (res.length !== 0) {
    ...
    const fetchData = async () => {
        ...
        dispatch(getMySavePost(morePostData)).unwrap().then((res) => {
        ...
        //     if (res.success === 1) {
        //         setMySavePost([...mySavePost, ...res.data])
    ...
    return (
        <div>
            ...
            <InfiniteScroll
                dataLength={savepost.length} //This is important field to render the next data
                next={fetchData}
                hasMore={hasmore}
